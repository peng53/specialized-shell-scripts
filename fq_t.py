...
def remove_top(f_name: str,f_dir: str) -> str:
	...
	if r:
		r = r.rstrip()
		add_bottom(q_hist_name,get_envvar('TMP'), r)
		return r
...
def case_default(args: List[str]) -> None:
	...
	url = args[0] if len(args)>1 else remove_top(q_name,get_envvar('TMP')) # type: str
...
def case_add(args: List[str]) -> None:
	...
	if len(args)>1 and len(args[1])>0:
		url = args[1]
		add_bottom(q_name,get_envvar('TMP'),url)
	else:
		print("URL was blank.")
...
def case_go(args: List[str],resume: bool = False) -> None:
	...
	url = args[1] if len(args)>1 else remove_top(q_name,get_envvar('TMP')) # type: str
	if not url:
		return
	yd = yt_info()
	...
	audf = get_audio_fmt(info, get_envvar("abr"))
	...
	vid = DownloadThread(url, vidf, os.path.join(get_envvar('TMP'),'1'), get_envvar('speed'), True)
	aud = DownloadThread(url, audf, os.path.join(get_envvar('TMP'),'1aud'), get_envvar('speed'), True)
